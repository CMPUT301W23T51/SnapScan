...
public class LoginActivityUiTest {
    ...
    //Test for unregistered account and register process. Please uninstall then reinstall the app before running test
    @Test
    public void testLoginUnregisteredAccount() throws Exception {
        ...
        Assert.assertTrue(radioButtonRegisterSelectGender.isChecked());
    ...
    //Test for newly registered account
    @Test
    public void testLoginRegisteredAccount() throws Exception {

        // make sure current activity is right.
        solo.assertCurrentActivity("Wrong Activity", LoginActivity.class);

        // type the email into the correct field
        solo.enterText((EditText) solo.getView(R.id.editText_login_email), randomEmail);
        Assert.assertTrue(solo.waitForText(randomEmail, 1, 2000));

        // type the password into the correct field
        solo.enterText((EditText) solo.getView(R.id.editText_loginPassword), randomPassword);
        Assert.assertTrue(solo.waitForText(randomPassword, 1, 2000));

        // wait for profile screen
        solo.clickOnView((Button) solo.getView(R.id.Button_login1));
        Assert.assertTrue(solo.waitForText("TOTAL QRS", 1, 2000));
    }

    //Test for registered account
    @Test
